...
11.[SECURITY][CRITIQUE] - Remplacer RSA PKCS#1 v1.5 par OAEP sur la poignee de main Ducky
            Chemin : ducky-network/key_decode_encode/DecodeWithServeurPrivateKey.go:25
                     ducky-network/key_decode_encode/EncodeWithClientPublicKey.go:47

            LE PROBLEME. rsa.DecryptPKCS1v15 est vulnerable a Bleichenbacher : un oracle
            de bourrage a texte chiffre choisi. Les trois conditions sont reunies ici :
              ...
              - tant que IsSafe est faux, le serveur dechiffre tout ce qu'on lui envoie
                avec sa cle privee (MessageReader) ;
              - l'echec est distinguable : log CRITICAL, et comportement different en aval.
            Un attaquant qui a enregistre un echange de cle de session peut la dechiffrer
            hors ligne en quelques millions de requetes, sans posseder aucun identifiant.

            CORRECTIF. Passer a rsa.EncryptOAEP / rsa.DecryptOAEP (sha256.New(), label nil).
            Attention : OAEP reduit la charge utile maximale. Pour RSA-4096 on passe de
            ~501 a ~446 octets ; verifier que les trames pre-IsSafe tiennent, sinon
            fragmenter.

            ATTENTION MIGRATION. C'est un changement de FORMAT DE TRAME, donc incompatible :
            un agent en PKCS1v15 ne peut pas parler a un serveur en OAEP. Il faut soit une
            periode ou le serveur tente OAEP puis retombe sur PKCS1v15 (mais le repli
            RECREE l'oracle, donc a borner dans le temps et a journaliser), soit une
            montee de version coordonnee du parc.

            REGLE PROJET : c'est une modification du protocole hors 05_XX. Documentation
            d'abord (Tableau_Protocole_Reseau.md), validation ensuite, code apres.
