...
func CoreFingerprintPath() string {
	return filepath.Join(store.KeyPath, FingerprintFileName)
}
...
func (e *ErrCleCoreInattendue) Error() string {
	return fmt.Sprintf(
		...
		e.Attendue, e.Recue,
		CoreFingerprintPath(), filepath.Join(store.KeyPath, "serveurpublickey.pem"))
}
...
func EcrireEmpreinte(empreinte string) error {
	...
	if err := os.MkdirAll(store.KeyPath, 0o700); err != nil {
		return err
	}
