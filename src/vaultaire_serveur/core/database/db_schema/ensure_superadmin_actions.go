...
func EnsureSuperadminActions(db *sql.DB, actionKeys []string) error {
	...
	permID, found, err := database.LookupUserPermissionID(db, isprotected.ProtectedUserPermission)
	if err != nil {
		return fmt.Errorf("lecture de la permission %s : %w", isprotected.ProtectedUserPermission, err)
	}
	if !found {
		...
		logs.Write_Log("WARNING",
			"bootstrap: permission "+isprotected.ProtectedUserPermission+" absente, actions non accordées")
		return nil
	}
	...
	if granted > 0 {
		logs.Write_Log("INFO", fmt.Sprintf(
			...
			granted, isprotected.ProtectedUserPermission, skipped))
	}
	return nil
}
