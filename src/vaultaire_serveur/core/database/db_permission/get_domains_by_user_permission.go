...
func Command_GET_Domains_ByUserPermission(db *sql.DB, permissionName string) ([]string, error) {
	...
	if err != nil {
		logs.Write_LogCode("ERROR", logs.CodeDBQuery, "database: "+"Erreur lors de la récupération des domaines pour la permission utilisateur '"+permissionName+"' : "+err.Error())
		return nil, err
	}
	defer func() {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&domain); err != nil {
			logs.Write_LogCode("ERROR", logs.CodeDBQuery, "database: "+"Erreur lors du scan des domaines pour la permission utilisateur '"+permissionName+"' : "+err.Error())
			return nil, err
		}
