...
// Rotation des journaux FICHIER du core, faite PAR LE PROGRAMME.
//
// # Ce que ce fichier fait tourner, et ce qu'il ne fait pas
//
// Le journal PRINCIPAL du core part sur la sortie standard — Twelve-Factor — et
// c'est systemd, journald ou le moteur de conteneurs qui le collecte, avec sa
// propre rétention. Le faire tourner ici en tiendrait deux, et « où sont les
// journaux » aurait deux réponses.
//
// Seules les familles qui passent par un fichier sont concernées : `date` et
// `SQL_Injection`, qu'on veut pouvoir lire séparément du flot général.
//
// # Pourquoi pas logrotate
//
// Parce qu'il faudrait l'installer, le configurer et le maintenir, et qu'un
// fichier de configuration système de plus est une chose de plus qui peut
// manquer, diverger ou être écrasée par un outil de gestion de configuration.
//
...
// # La politique, en dur
//
// Ce ne sont pas des réglages : ce sont des valeurs par défaut saines. Aucun
// fichier de configuration, aucune clé en base, aucune variable
// d'environnement.
//
...
// `duckynetwork/logs`. Un test de chaque côté fige les mêmes valeurs, sinon les
// deux dérivent en silence.
...
var (
	...
	//
	// 50 Mo. Ces deux familles écrivent peu — quelques lignes par jour sur un
	// core sain — mais un scan qui déclenche l'assainissement des requêtes en
	// rafale fait de SQL_Injection.log le fichier qui grossit le plus vite de
	// la machine, et c'est exactement le moment où l'on veut qu'il tourne.
	TailleMaxJournal int64 = 50 << 20
