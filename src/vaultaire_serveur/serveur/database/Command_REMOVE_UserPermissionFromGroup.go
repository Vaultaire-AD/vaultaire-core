...
func Command_Remove_UserPermissionFromGroup(db *sql.DB, groupName, permissionName string) error {
	...
	// Vérifier si la permission est déjà attribuée au groupe (à adapter selon ta nouvelle table)
	var count int
	queryCheck := `SELECT COUNT(*) FROM group_permission_user WHERE d_id_group = ? AND d_id_permission = ?`
	...
	queryRemove := `DELETE FROM group_permission_user WHERE d_id_group = ? AND d_id_permission = ?`
