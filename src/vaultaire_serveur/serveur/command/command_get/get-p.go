...
func get_Permission_Command_Parser(command_list []string) string {
	if len(command_list) == 2 {
		...
		case "-u":
			permission, err := database.Command_GET_AllUserPermissions(database.GetDatabase())
			if err != nil {
	...
	if len(command_list) == 3 {
		...
		case "-u":
			permission, err := database.Command_GET_UserPermissionByName(database.GetDatabase(), command_list[2])
			if err != nil {
				logs.Write_Log("WARNING", "error during the get of all permissions : "+err.Error())
				return (">> -" + err.Error())
			}
